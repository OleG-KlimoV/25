...
def test_all_my_pets_have_name_age_type(testing):
    pytest.driver.find_element(By.LINK_TEXT, "Мои питомцы").click()
    ...
    # неявные ожидания элемента type
    pytest.driver.implicitly_wait(10)
    type = pytest.driver.find_elements(By.XPATH, '//*[@id="all_my_pets"]/table[1]/tbody[1]/tr/td[2]')
...
def test_no_duplicate_pets_in_the_list(testing):
    ...
    description_pet = []
    for i in range(len(names)):
        description_pet.append(names[i].text + '_' + (type[i].text) + '_' + (age[i].text))
